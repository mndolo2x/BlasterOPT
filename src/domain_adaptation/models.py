"""
Pydantic Data Models for Geology Domain Adaptation Package.
"""
...
class DomainAdaptationConfig(BaseModel):
    ...
    enable_dann: bool = Field(default=False, description="Whether to enable Domain-Adversarial Neural Network (DANN) adaptation if fine-tuning target R2 < target_r2_threshold")
...
class FineTuneMetrics(BaseModel):
    ...
    method_used: str = Field(..., description="Adaptation method used: 'FINE_TUNING' or 'DANN_ADVERSARIAL'")
...
class DomainAdaptationReport(BaseModel):
    ...
    metrics: FineTuneMetrics = Field(..., description="Metrics summary")
